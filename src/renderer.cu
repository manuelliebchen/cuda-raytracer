...
Renderer::readeSceneFile( std::string file) {
  ...
  if(value) {
    ...
    cameraDirection = cameraDirection.normalize() * (float) cameraDoc[L"focallength"]->AsNumber();
    camera = Camera( cameraPosition, cameraDirection);

    JSONObject json_image = document[L"image"]->AsObject();
    image = Image(json_image[L"width"]->AsNumber(), json_image[L"height"]->AsNumber());

    JSONObject settings = document[ L"settings"]->AsObject();
    multisamples = settings[L"multisamples"]->AsNumber();

    scene.parseFile( document[L"material"]->AsArray(), document[L"scene"]->AsArray());
  } else {
...
Renderer::runOnHost() {
  ...
  int multisamples_pattern[42] = {  0, 0,
                                   -2,-6, 6,-2,-6, 2, 2, 6,
  ...
  for( unsigned int i = 0; i < image.getHeight(); ++i) {
    for( unsigned int j = 0; j < image.getWidth(); ++j) {
      ...
      for( unsigned int k = 0; k < multisamples; ++k) {
        ...
        Ray ray = camera.getRay( make_float2( ((imagePosition.x) - (0.5f * image.getWidth()) + ((float) multisamples_pattern[pattern_start+k*2]) / 16.0f) / (pixel_angle),
                                              ((imagePosition.y) - (0.5f * image.getHeight()) + ((float) multisamples_pattern[pattern_start+k*2+1]) / 16.0f) / (pixel_angle)));
      ...
      uchar3 color = make_uchar3( (unsigned char) (floatColorSummarized.x* 255.0f), (unsigned char) (floatColorSummarized.y* 255.0f), (unsigned char) (floatColorSummarized.z * 255.0f));
...
Renderer::runOnDevice() {
  ...
  cudaMemcpy( (void*) deviceRenderer, (void*) lokalDeviceRenderer, sizeof(DeviceRenderer), cudaMemcpyHostToDevice);

  cudaCheckError();

  unsigned int threadDimension = floorf( sqrt(deviceProperties.maxThreadsPerBlock));
  dim3 threadsPerBlock( threadDimension, threadDimension, 1);

  dim3 numberOfBlocks( (unsigned int) std::ceil( (float) ( multisamples * image.getWidth()) / (float) (threadsPerBlock.x)),
                       (unsigned int) std::ceil( (float) ( multisamples * image.getHeight()) / (float) (threadsPerBlock.y)), 1);

  auto start_device = std::chrono::high_resolution_clock::now();
  globalRender <<< numberOfBlocks, threadsPerBlock, lokalDeviceRenderer->getSharedMemory() >>> (deviceRenderer);
  ...
  cudaMemcpy( (void*) lokalDeviceRenderer, (void*) deviceRenderer, sizeof(DeviceRenderer), cudaMemcpyDeviceToHost);
...
Renderer::runOnDeviceSingleThreadet() {
  ...
  auto start_device = std::chrono::high_resolution_clock::now();
  globalRenderSingleThreadet <<< 1, 1, lokalDeviceRenderer->getSharedMemory() >>> (deviceRenderer);
  cudaDeviceSynchronize();
  cudaCheckError();
