...
__host__ __device__ vec3
vec3::reflect(const vec3& rhs, const vec3& normal)
{
  return rhs - normal * (2 * vec3::dot(rhs, normal));
};

__host__ __device__ vec3
vec3::refract(const vec3& rhs, const vec3& normal, float refractionIndex)
{
  // TODO
  return vec3();
};
...
{
  os << "vec3: x=" << std::to_string(vec.x) << " y=" << std::to_string(vec.y)
