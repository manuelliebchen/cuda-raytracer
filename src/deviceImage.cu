...
DeviceImage DeviceImage::operator= (const Image& _image) {
  width = _image.width;
  height = _image.height;
  memory = _image.memory;

  cudaMalloc( (void**) &deviceData, memory * sizeof(unsigned char));
  cudaMemcpy( (void*) deviceData, _image.hostData, memory * sizeof(unsigned char), cudaMemcpyHostToDevice);

  return (*this);
};

size_t DeviceImage::getMemoryUsage() const {
  return memory;
}
...
__device__ unsigned DeviceImage::getWidth() const{
  return width;
};

__device__ unsigned DeviceImage::getHeight() const{
  return height;
};

__device__ void DeviceImage::setPixel( uint2 position, uchar3 value) {
