...
Scene::phong(const Intersection& intersection) const
{
  ...
  if (material.getEmission() == 0) {
    float phong = 0.1;
    for (int i = 0; i < hostPlanes.size(); i++) {
      const Material& emMaterial =
        hostMaterials[hostPlanes[i].getMaterialIndex()];
      if (emMaterial.getEmission() > 0) {

        vec3 to_light =
          (hostPlanes[i].getEmissionPoint() - intersection.position);

        Intersection shadow_intersection;
        Ray ray(intersection.position + to_light.normalized() * EPSILON,
                to_light.normalized(),
                ((float)to_light) - EPSILON * 2);
        if (trace(ray, shadow_intersection)) {
          continue;
        }

        float kd = vec3::dot(to_light.normalized(), intersection.normal) *
                   material.getDiffuse();
        kd = std::max(0.0f, kd);
        float ks = vec3::dot(to_light.normalized(), intersection.reflection);
        if (ks > 0) {
          ks = std::pow(ks, 32) * material.getSpecular();
        } else {
          ks = 0;
        }
        phong += kd + ks;
...
void
Scene::parseFile(JSONArray material, JSONArray objects)
{
  for (JSONValue* i : material) {
    JSONObject v = i->AsObject();
    JSONArray color = v[(L"color")]->AsArray();
    hostMaterials.push_back(Material(make_float3(color[0]->AsNumber(),
                                                 color[1]->AsNumber(),
                                                 color[2]->AsNumber()),
