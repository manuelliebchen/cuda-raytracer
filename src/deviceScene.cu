...
{
  totalMemory = numSpheres * sizeof(Sphere) + numPlanes * sizeof(Plane) +
                numMaterials * sizeof(Material);
  cudaMalloc((void**)&memory, totalMemory);

  cudaMemcpy(memory,
             _scene.hostSpheres.data(),
             numSpheres * sizeof(Sphere),
             cudaMemcpyHostToDevice);
  cudaMemcpy(memory + numSpheres * sizeof(Sphere),
             _scene.hostPlanes.data(),
             numPlanes * sizeof(Plane),
             cudaMemcpyHostToDevice);
  cudaMemcpy(memory + numSpheres * sizeof(Sphere) + numPlanes * sizeof(Plane),
             _scene.hostMaterials.data(),
             numMaterials * sizeof(Material),
             cudaMemcpyHostToDevice);
};
