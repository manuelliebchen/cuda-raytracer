...
__syncthreads();
  ...
  float2 camera_position = make_float2(position % (blockDim.x/multi_sqrt), position / (blockDim.x/multi_sqrt));

  float pixel_angle = (float) (image.getWidth() + image.getHeight()) / 2;
  Ray ray = camera.getRay( make_float2( ((float)(camera_position.x + blockDim.x * blockIdx.x/multi_sqrt) - (0.5f * image.getWidth()) + ((float) multisamples_pattern[pattern_start+sample_index*2]) / 16.0f) / pixel_angle,
                                        ((float)(camera_position.y + blockDim.y * blockIdx.y/multi_sqrt) - (0.5f * image.getHeight()) + ((float) multisamples_pattern[pattern_start+sample_index*2+1])/ 16.0f) / pixel_angle));
...
__syncthreads();
  ...
  if( linearThreadId < 1024 / multisamples){
    ...
    uint2 imagePosition = make_uint2( camera_position.x + (blockDim.x * blockIdx.x/multi_sqrt), camera_position.y + (blockDim.y * blockIdx.y/multi_sqrt));
