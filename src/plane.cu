...
Plane::intersect(Ray& ray, Intersection& intersection) const
{
  // Calculate Normal
  ...
  float t_new = (vec3::dot(normal_plane, position - ray.origin)) /
                (vec3::dot(normal_plane, ray.direction));
  ...
  ray = new_ray;
  intersection =
    Intersection((vec3)ray, normal_plane, ray.direction.normalized(), material);

  return true;
};

__host__ __device__ vec3
Plane::getEmissionPoint() const
{
