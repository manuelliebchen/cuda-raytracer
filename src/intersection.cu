...
__host__ __device__
Intersection::Intersection(  vec3 _position, vec3 _normal, vec3 _in, size_t _materialIndex) :
  position( _position),
...
__host__ __device__ Intersection Intersection::operator= ( const Intersection& intersection) {
  position = intersection.position;
  normal = intersection.normal.normalized();
  in = intersection.in.normalized();
  materialIndex = intersection.materialIndex;
